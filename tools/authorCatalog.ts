...
entries.push(
  entry({
    ...
    actions: [A.short, A.long, A.switch],
  ...
  entry({
    id: "GK_BUILD_02", role: "GK", category: "on_ball", phase: "build_up", title: "Distribution against a press",
    trigger: { all: [...ON, c("nearestOppDist", "<", 12)] },
    cues: ["Who is pressing and from where?", "Which short lane is still open?", "Is the long option worth the risk?"],
    actions: [A.short, A.long, A.hold],
    ...
    positive: { hasBall: 1, nearestOppDist: 7 }, negative: { hasBall: 1, nearestOppDist: 15 },
  }),
  entry({
  ...
  entry({
    id: "GK_SHOT_01", role: "GK", category: "defending", phase: "defending", title: "Attacker closing on goal",
    trigger: { all: [...DEF, c("carrierDistToOurGoal", "<", 22)] },
    cues: ["Is the shot angle open?", "Is a defender delaying them?", "Can you narrow the angle safely?"],
    ...
    positive: { theirPossession: 1, carrierDistToOurGoal: 15, carrierDist: 10 }, negative: { theirPossession: 1, carrierDistToOurGoal: 40 },
  }),
  entry({
...
entries.push(
  ...
  entry({
    ...
    actions: [A.recycle, A.gap, A.space, A.hold],
  ...
  entry({
    ...
    actions: [A.gap, A.recycle, A.space, A.hold],
...
entries.push(
  ...
  entry({
    ...
    actions: [A.recycle, A.gap, A.hold, A.space],
...
entries.push(
  entry({
    ...
    actions: [A.gap, A.space, A.draw, A.switch, A.shoot],
  ...
  entry({
    ...
    cues: ["Where is the free teammate?", "Is there a gap behind the presser?"],
    actions: [A.recycle, A.gap, A.hold, A.space],
  ...
  entry({
    ...
    trigger: { all: [...ON, c("secondsSinceTurnover", "<", 3)] },
    cues: ["Are they disorganised?", "Is the striker ready to run?", "Is a shot on?"],
    actions: [A.gap, A.space, A.recycle, A.shoot],
...
function winger(role: RoleId, mirror: RoleId | null): CatalogEntry[] {
  ...
  return [
    mk({
      ...
      cues: ["Which foot is the defender on?", "Is the full back overlapping?", "Is the cutback on?"],
      actions: [A.space, A.draw, A.gap, A.recycle, A.shoot],
    ...
    mk({
      ...
      cues: ["Is the far-side defender watching the ball?", "Is a switch possible?", "Is there room behind?"],
      actions: [A.width, A.runBehind, A.underneath],
      mistakes: ["Drifting inside and closing the switch", "Running offside early"],
      difficultyFactors: ["Switch availability", "Space behind"],
      continuation: "Switch found, run behind, or shape.",
...
entries.push(
  entry({
    ...
    actions: [A.shoot, A.gap, A.space, A.draw],
  ...
  entry({
    ...
    cues: ["Who is arriving underneath?", "Can you turn?", "Is a winger running?"],
    actions: [A.recycle, A.hold, A.space, A.gap],
